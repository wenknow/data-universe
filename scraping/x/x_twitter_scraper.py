...
class XTwitterScraper(Scraper):
    """
    Scrapes tweets using the Microworlds Twitter Scraper: https://console.apify.com/actors/angq0AOHkjbCU1Yo8.
    """

    ACTOR_ID = "DDe3hNSgaR7I1wkOc"
    ...
    async def validate(self, entities: List[DataEntity]) -> List[ValidationResult]:
        ...
        async def validate_entity(entity) -> ValidationResult:
            ...
            while attempt < max_attempts:
                ...
                run_input = {
                    **XTwitterScraper.BASE_RUN_INPUT,
                    "urls": [entity.uri],
                    "maxTweets": tweet_count,
                }
                run_config = RunConfig(
                    actor_id=XTwitterScraper.ACTOR_ID,
        ...
        with XTwitterScraper.concurrent_validates_semaphore:
            bt.logging.trace(
    ...
    async def scrape(self, scrape_config: ScrapeConfig) -> List[DataEntity]:
        ...
        query = random.choice(scrape_config.labels).value
        if query.startswith("#"):
            query = query[1:]
        fromdate = scrape_config.date_range.start.astimezone(tz=dt.timezone.utc).strftime(date_format)
        untildate = scrape_config.date_range.end.astimezone(tz=dt.timezone.utc).strftime(date_format)
        ...
        run_config = RunConfig(
            ...
            timeout_secs=XTwitterScraper.SCRAPE_TIMEOUT_SECS,
        )

        bt.logging.trace(f"Performing Twitter scrape for search terms: {query}.")
        bt.logging.info(f"---------------scrape_config---------------: {scrape_config}.")
        bt.logging.info(f"---------------run_input---------------: {run_input}.")
        bt.logging.info(f"---------------run_config---------------: {run_config}.")
